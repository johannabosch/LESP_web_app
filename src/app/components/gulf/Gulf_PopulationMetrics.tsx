...
const Gulf_PopulationMetrics: React.FC<Gulf_PopulationMetricsProps> = ({ selectedSite }) => {
  ...
  // Allow a larger resizing by setting a higher maxBoxWidth
  const maxBoxWidth = Math.min(1500, containerWidth + 500); // Increase max width limit
  const maxBoxHeight = Math.min(800, containerHeight - 150); // Max height is container minus some padding
