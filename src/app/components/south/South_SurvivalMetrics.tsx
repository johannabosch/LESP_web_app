...
// South Survival Metrics component
const South_SurvivalMetrics: React.FC<{ selectedSite: string }> = ({ selectedSite }) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const headerRef = useRef<HTMLParagraphElement>(null);
  const [containerWidth, setContainerWidth] = useState<number>(window.innerWidth);
  const [containerHeight, setContainerHeight] = useState<number>(window.innerHeight);
  const [southData, setSouthData] = useState<any[]>([]); // State to store the JSON data
  ...
  // Fetch the South survival table data from the JSON file
  useEffect(() => {
    fetch("/figures/South/survival_table_SNS.json")
      .then((response) => response.json())
      .then((data) => setSouthData(data))
      .catch((error) => console.error("Error fetching South survival data:", error));
  }, []);
  ...
  return (
    <div ref={containerRef} className="block" style={{ height: '100%', overflow: 'auto' }}>
      ...
      <p ref={headerRef} className="bg-[#B4D6F7] rounded font-bold text-center p-[4px] text-white text-2xl"
